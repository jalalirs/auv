...
func (g GrantSpec) Validate() error {
	...
	case ScopeOrg, ScopeCity, ScopeVehicle, ScopeQueue:
		if g.ScopeID == "" {
			return fmt.Errorf("%w: a %s binding names its scope", domain.ErrInvalid, g.ScopeKind)
		}
	default:
		return fmt.Errorf("%w: a binding's scope is the platform, an organisation, a city, a vehicle, a queue, or work, not %q",
