...
def test_a_colony_stands_the_right_way_up_and_the_right_size():
    ...
    rather than what the specimen happened to be."""
    points, _ = scanned.as_a_colony(SPECIMEN, 0.8)
    assert abs(float(np.ptp(points[:, :2], axis=0).max()) - 0.8) < 0.01
    assert abs(float(points[:, 2].min())) < 1e-6, "it should sit on the ground"
    assert float(points[:, 2].max()) > 0.0, "and stand up from it"


@have_it
def test_it_is_centred_on_its_own_footprint():
    """So that where a reef puts a colony is where the colony is."""
    points, _ = scanned.as_a_colony(SPECIMEN, 1.0)
    assert abs(float(points[:, 0].mean())) < 0.02
    assert abs(float(points[:, 1].mean())) < 0.02
